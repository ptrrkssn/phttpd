...
#define SERVER_BIN	INSTROOT "/sbin/phttpd"
...
struct options global_cfg_table[] =
{
    ...
    { "config-file",	   T_FILE,   NULL,			NULL, 'f', 0, NULL, "Path to additional config file" },
    { "primary-config-file",	   T_FILE,   NULL,		NULL, 'F', 0, NULL, "Path to primary config file" },
    
    { "database-dir",      T_STRING, &server_home,		NULL, 0, 0, NULL, "Path to database directory" },
    { "sub-database-dir",  T_STRING, &sub_server_home,		NULL, 0, 0, NULL, "postfix pathpart for virtualservers" },
    ...
    { "virtual-admin-email",   T_STRING, &virtual_admin_email,          NULL, 0, 0, NULL,  "Webmaster's relative address for hostingmodes" },
    ...
    { "isdaemon",	   T_NUMBER, &already_daemon,		NULL, 0, 0, NULL, "Is already in daemon mode level" },
    ...
    { "restart-requests",  T_NUMBER, &restart_requests,		NULL, 0, 0, NULL, "Number of requests to server befor refork" },
    { "restart-grace-time",T_NUMBER, &restart_grace_time,	NULL, 0, 20, NULL, "Grace time, an old process wait for connections to finish" },
    ...
    { "mode-dotted",       T_BOOL, &mode_dotted,		NULL, 0, 0, NULL, "Multimode URL expanding hex or dotted ipnr"},
...
{
    printf("[Phttpd %s - Copyright (c) 1995-1997 Peter Eriksson <pen@signum.se> 1998-1999 Roland Kaltefleiter <rk@netuse.de>]\n\n",
    server_version);
}
...
static int setup_uidgid(void)
{
    int x;
    if ( debug > 3 )
      fprintf(stderr,"phttpd: Calling initgroups (%s / %ld )\n",server_user,server_gid);
    x=initgroups(server_user,server_gid);
      fprintf(stderr,"phttpd: back from initgroups\n");
...
int main(int argc, const char *argv[])
{
	...
	{
	    if (argv[i][2])
		default_cfg_file=strdup(argv[i]+2);
	}
    }

    if ( default_cfg_file == NULL )
	default_cfg_file=strdup(CONFIG_FILE);
    
    fd_init();
    
    config_parse_file(default_cfg_file, global_cfg_table, 0);
...
/* HACK we need to make sub_server_home nice */
   if ( sub_server_home == NULL ) sub_server_home=s_strdup("");
   else
   {
	if ( sub_server_home[0]!='/' )
	{
		fprintf(stderr, "phttpd: sub_server_home MUST have a leading / .\n");
		syslog(LOG_ERR, "sub_server_home MUST have a leading / .");
		exit(1);
	}
   }

   if ( restart_requests < 10000 && restart_requests > 0 )
	{
		fprintf(stderr,"restart-requests is TOO low (%d), should be > 10000 !\n",restart_requests);
		syslog(LOG_ERR, "WARNING: restart-requests is TOO low, should be > 10000 !");
	}
    
   if ( server_addr == NULL)
	{ server_addr=s_strdup("*"); }
	...
	if (server_port != 80)
	    printf("Server will be started on %s:%d as (%d) with debuglevel %d\n",
	    server_host,
	    server_port,
	    (int) getpid(),
	    debug);
	...
	switch (sig)
	{
	  ...
	  case SIGQUIT:  /* restart totally */
            if (debug > 0 )
                fprintf(stderr, "** Got SIGQUIT(3) - restaring\n");

/*	    sigrelse(SIGTERM); */
            syslog(LOG_NOTICE, "server restarted by SIGQUIT(3) request");
/* set the global we must end ! */
	    do_restart=1;
/* also force close of all active sessins when finished: NO KEEPALIVE */
	    keepalive_enabled=0;
...
/* we only need one LWP */
	    npid=vfork();
	    if ( npid == 0 )
/* we are in the child now */
		{
/* restore root if avail */
			nargv=calloc(argc+2,sizeof(char *));
			nargv[0]=strdup(argv[0]);
			fprintf(stderr,"%d: nargv[0]: %s\n",argc,nargv[0]);
			nargv[1]=strdup("--isdaemon=1");
			fprintf(stderr,"nargv[1]: %s\n",nargv[1]);
			for(i=2;i<=argc;i++)
			...
			if ( argv[0][0]=='/' )
				...
				execv(SERVER_BIN,nargv);
			exit(99);
		}

		while ( (leftover=cn_active()) > 0 && restart_grace_time > 0 );
	    	{
			fprintf(stderr, "** wait %d seconds for last %d connects to finish\n",restart_grace_time,leftover);
			sleep(1);
			restart_grace_time--;
		}
/* force close all */
	    	cn_closeall();
/* be nice, wait for close to be really done */
	    	sleep(1);
	    	...
	    	leftover=cn_active();
            	if ( leftover > 0 )
			fprintf(stderr, "** ups, could not close %d connects\n",leftover);

           	fprintf(stderr, "** old process will die now\n");
		exit(0);
            break;
