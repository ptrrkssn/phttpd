				  ...
				  int *h_errnop)
{
    ...
    mutex_lock(&nss_lock);
    if ( debug > 6 )
	    fprintf(stderr,"ERR-A-DNS: %lx (%d) %d -- %d %d\n",*(long *)addr,length,*h_errnop,errno,buflen);
    hep = gethostbyaddr_r(addr,
			  ...
			  h_errnop);
    if ( debug > 6 )
	    fprintf(stderr,"ERR-B-DNS: %lx (%d) %d -- %d\n",*(long *)addr,length,*h_errnop,errno);
