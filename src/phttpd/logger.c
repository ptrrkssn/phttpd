		    ...
		    int result)
{
	...
	char *url, *request, *hostname, *address;
	...
	if (hip->aip)
	{
	    auth_user = hip->aip->validated_username;
	}
	if (auth_user)
	    auth_user = url_quote(auth_user, buf5, sizeof(buf5), "", 0);

	ident_user = NULL;
	(void) ident_get(cip->ident, NULL, NULL, &ident_user);
	...
	if (hip->length)
	    if (logheadervolume)
	    	fd_printf(log_fd, "%d", headerbytes+hip->length);
	    else
    ...
    if ( result > 0 && result < 400 && count_fd != NULL && hip->orig_url)
    {
