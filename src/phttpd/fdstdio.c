...
** If size != 0, use a maximum size limit of buffer
*/
char *fd_gets_nl(char *buf, int size, int fd)
{
    int c, i;
    int r=0;
...
static void *relay_data(struct relayinfo *rip)
{
    ...
    if (debug > 6)
	fprintf(stderr, "fdstdio: relay_data(%d -> %d): Start[%d]\n",
		rip->read_fd, rip->write_fd,rip->bytes);
...
int fd_prelay(int fd1, int fd2, int bidir, int bytes)
{
    thread_t tid1, tid2;
    struct relayinfo rb1, rb2;
