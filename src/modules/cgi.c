...
static int run_as_docrootowner = 0;
static uid_t min_run_uid = 100;
...
static struct options cgi_cfg_table[] = 
{
    ...
    { "run-as-docrootowner", T_BOOL, &run_as_docrootowner, NULL },
    { "minimum-run-uid", T_NUMBER, &min_run_uid, NULL },
...
/* first set new_root from cgi-newroot-table  and as fallback from cgi_newroot */
	...
	{
/* ###                     none|basedir|extended|svsextended|effective|withsubdir
   ### auto-newroot-mode =  0      1       2         3           4         5
   ###
 */
		...
		if ( auto_newroot_mode == 3 && hip->svsname != NULL )
			{
				s_sprintf(new_root_buf,MAXPATHLEN,"%s/%s",server_home,hip->svsname);
				new_root=s_strdup(new_root_buf);
			}
		if ( auto_newroot_mode == 4 && hip->prefix != NULL )
			{
				s_sprintf(new_root_buf,MAXPATHLEN,"%s/%s",server_home,hip->prefix-strlen(sub_server_home));
				new_root=s_strdup(new_root_buf);
			}
		if ( auto_newroot_mode == 5 && hip->prefix != NULL )
			{
				s_sprintf(new_root_buf,MAXPATHLEN,"%s/%s",server_home,hip->prefix);
				new_root=s_strdup(new_root_buf);
			}
		if ( debug > 6 ) fprintf(stderr,"new_root=%s\n",(new_root)?new_root:"<NULL>");
	}
      else
...
/* check uid-gid table..... */
    {
	if ( run_as_docrootowner == 1 )
	{
		s_sprintf(path2,MAXPATHLEN,"%s/%s",server_home,hip->prefix);
		if ( stat(path2,&sb) == -1 ) return (-403);
		uid=sb.st_uid;
		gid=sb.st_gid;
	}
	else
	{
		...
		{ 
			if ( debug > 4 )
				fprintf(stderr,"cgi2: checking....cgi_uidgid_table\n");
			if ( url_match(cgi_uidgid_table,url,rewrite_pattern, sizeof(rewrite_pattern)))
				{
					if ( debug > 4 ) 
						fprintf(stderr,"cgi2: got %s\n",rewrite_pattern);
					rwp2=strchr(rewrite_pattern,':');
					if (rwp2!=NULL) 
					{
						if (strlen(rwp2)>1) {*rwp2=0x00; rwp2++; }
						else rwp2=NULL;
					}
					if ( uidgid_get(rewrite_pattern,rwp2,&uid,&gid,&pwp, &pwb, pbuf, sizeof(pbuf)) < 0)
              				{ 
	...
	} /* end of if run_as_docrootowner else-part */
...
/* added to enable chdir in bindir for cgi */
    ...
    if ( min_run_uid == -1 || min_run_uid <= uid )
    	pid = proc_run(path,
		   uid, gid,
		   new_root,
	...
	{
		pid=-1;
		fprintf(stderr, "cgi.c: min_run_uid > uid ! exec denied !\n");
	}
