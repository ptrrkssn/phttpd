...
static struct options dir_cfg_table[] =
{
    ...
    { "check-index-file",  T_BOOL,   &check_index_file,	NULL },
			 ...
			 int browse_flag)
{
    ...
    if (!browse_flag)
	html_sysheader(fd, "H2", "Index of %s%s", make_sip_url(cip,qbuf,2048), hip->url+hip->prelen);
...
/* rkmultimode hip->prelen */
		  url_quote(url_getparent(hip->url+hip->prelen, buf, sizeof(buf)),
                            qbuf,2048,NULL,1) );
    }
	...
	if (i > 0)
	{
		...
		else
		{
		    ...
		    fd_printf(fd, " <A HREF=\"%s/\">%s</A>",
			      url_quote(fdpl[j]->dbp->d_name,qbuf,2048,NULL,1),
			      fdpl[j]->dbp->d_name);
...
static int http_get_head(struct connectioninfo *cip)
{
    int len, status;
    char url[2048];
    char url2[2048]; /* added rk 9.4.98 try */
    char *orig_url;
    struct stat sb;
    int fd = cip->fd;
    ...
    {
	s_strcpy(url, sizeof(url), cip->sip->url);
...
/* Hack RK -- we do need this ? or not ?*/
/* check-index-file ?? */
	    if ( check_index_file )
/*		{ if ( stat(url+1,&sb) != 0 ) goto nextfile; } */
		{ if ( url_expand(url,url2,sizeof(url2),&sb,NULL,NULL) == NULL ) goto nextfile; }

	    if ( url_getrewrite(url,url2,2048) != NULL ) 
		hip->url = url2;
	    else
	    	hip->url = url;
/* end of HACK */
