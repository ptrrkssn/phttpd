h41570
s 01910/00000/00000
d D 1.1 97/10/26 21:59:26 rk 1 0
c date and time created 97/10/26 21:59:26 by rk
e
u
U
f e 0
...
#if 1
		cp->block = NULL;
		dns_get(cip->server, NULL, NULL, &cp->block, NULL);
#else
...
#if 1
		cp->block = NULL;
		dns_get(cip->server, NULL, &cp->block, NULL, NULL);
#else		
...
#if 1
		cp->block = NULL;
		dns_get(cip->client, NULL, NULL, &cp->block, NULL);
#else
...
#if 1
		cp->block = NULL;
		dns_get(cip->client, NULL, &cp->block, NULL, NULL);
#else
			    ...
			    char *path_translated)
{
	...
	cgi_setenv(envp, "SERVER_PROTOCOL", hip->version);
        if ( !rkmultimode)
          cgi_setenv(envp, "SCRIPT_NAME", url);
        else
		     ...
		     struct includeinfo *ip)
{
	...
	if (strcmp(ap->name, "cmd") == 0)
	{
		...
		if (!rkmultimode)
		  cgi_setenv(envp, "DOCUMENT_URI", ip->cip->hip->url);
		else
		  cgi_setenv(envp, "DOCUMENT_URI", ip->cip->hip->url+ip->cip->hip->prelen);
		...
		{
		    cp->block = "[Unable to lseek on temporary file]";
		    cp->size = strlen(cp->block);
		    s_close(tmp_fd);
		    break;
		}
		
		if (fstat(tmp_fd, &sb) < 0)
		{
		...
		if ((cp->size = s_read(tmp_fd, cp->block, sb.st_size)) < 0)
		{
		    s_free(cp->block);
		    cp->free_start = NULL;
