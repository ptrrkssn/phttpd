h23165
s 00974/00000/00000
d D 1.1 97/10/26 21:59:25 rk 1 0
c date and time created 97/10/26 21:59:25 by rk
e
u
U
f e 0
			    ...
			    char *path_translated)
{
	...
	{
	    int blen = strlen( (cgi_newroot==NULL) ? server_home : "/" ) + 
	      strlen(path_translated) + 2;
	    
	    buf = (char *) alloca(blen);
	    s_strcpy(buf, blen, (cgi_newroot==NULL) ? server_home : "/" );
    ...
    if ( !rkmultimode)
      cgi_setenv(envp, "PHTTPD_ORIG_URL", hip->orig_url);
    else
      cgi_setenv(envp, "PHTTPD_ORIG_URL", hip->orig_url+hip->prelen);
...
/* NOT inside serverroot, seems to be okay here ??? */
    if (debug > 3)
	...
	if ( rkmultimode==0 )
	    s_sprintf(path,sizeof(path),"%s/%s",cgi_basedir,path2);
	else
