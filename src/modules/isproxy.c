...
int pm_request(struct connectioninfo *cip)
{
    ...
    address = NULL;
    (void) dns_get(cip->client, NULL, NULL, &address, NULL);
    
    write_data(sock_fd, hip, url_buf, address, cip->sip->url);
    if (hip->mip)
    {
    ...
    if ( strcasecmp(hip->method,"POST")==0 ) 
	{
    		fd_prelay(sock_fd, fd, 1, content_length+1);
	}
    else
	{
